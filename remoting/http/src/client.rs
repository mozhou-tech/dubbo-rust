...
use crate::{Code, Http2Config, Request, Response, Status};
    ...
    U: crate::message::RecvEntryMessage + 'static,
{
    ...
    fn call(&self, cx: &mut ClientContext, req: Request<T>) -> Self::Future
where {
...
mod tests {
    ...
    fn test_build_uri_ip() {
        ...
        let uri = "http://127.0.0.1:8000/path?query=1"
            .parse::<hyper::Uri>()
            .unwrap();
        assert_eq!(super::build_uri(Address::Ip(addr), path), uri);
    }
    ...
    fn test_build_uri_unix() {
        ...
        let uri = "http+unix://2f746d702f7270632e736f636b/path?query=1"
            .parse::<hyper::Uri>()
            .unwrap();
        assert_eq!(super::build_uri(Address::Unix(Cow::from(addr)), path), uri);
    }
}
