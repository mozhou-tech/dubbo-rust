...
use hyper::{http, Client as HyperClient, Request, Response};
use remoting_base::message::{BaseRecvMessage, BaseSendMessage};
use remoting_net::dial::Config;
use remoting_net::Address;
use tower::{util::ServiceExt, Service as TowerService, Service};
...
impl<T, U> Service<Request<T>> for ClientTransport<U>
where
    T: BaseSendMessage + Send + 'static,
    U: BaseRecvMessage + 'static,
{
...
mod tests {
    ...
    fn test_build_uri_ip() {
        ...
        let uri = "http://127.0.0.1:8000/path?query=1"
            .parse::<hyper::Uri>()
            .unwrap();
        assert_eq!(super::build_uri(Address::from(addr), path), uri);
    }
    ...
    fn test_build_uri_unix() {
        ...
        let uri = "http+unix://2f746d702f7270632e736f636b/path?query=1"
            .parse::<hyper::Uri>()
            .unwrap();
        assert_eq!(super::build_uri(Address::from(Cow::from(addr)), path), uri);
    }
}
